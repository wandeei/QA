...
class TopicDetailView(View):
    def get(self, request, pk):
        ...
        no_following_topic = len(TopicFollowing.objects.filter(follows=topic))
        total_answers = 0
        for question in questions_under:
            total_answers += len(Answer.objects.filter(question=question.question))
        user = UserOtherDetails.objects.get(user=request.user)
        follows = Topic.objects.get(pk=pk)
